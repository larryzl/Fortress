...
from users.models import Department,auth_gid
# from Fortress.settings import EMAIL_PUSH
import hashlib
...
class DepartmentEdit(View):
    ...
    template_name = 'users/department_edit.html'
    def get(self,request,**kwargs):
        uid = kwargs.get('uid',None)
        if uid:
            self.title[1] = '修改部门信息'
            department_obj = Department.objects.get(uuid=uid)
            form = DepartmentForm(instance=department_obj)
        else:
